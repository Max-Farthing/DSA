...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    protected void downHeap(int index) {
        ...
        if (leftIndex < size() && compare(list.get(leftIndex).getKey(), list.get(smallestChildIndex).getKey()) < 0) {
            smallestChildIndex = leftIndex;
        }
        if (rightIndex < size() && compare(list.get(rightIndex).getKey(), list.get(smallestChildIndex).getKey()) < 0) {
